...
class _UserMasterScreenState extends ConsumerState<UserMasterScreen> {
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _users = data.map((e) => UserProfile.fromJson(e)).toList());
      }
    } on TimeoutException {
  ...
  Future<void> _saveUser() async {
    ...
    try {
      ...
      if (_pickedImage != null) {
        ...
        currentAvatarUrl = await SupabaseService.uploadAvatar(userId, bytes, ext)
      ...
      if (mounted) {
        _showSuccess(_selectedUser == null ? 'User created successfully.' : 'User updated successfully.');
  ...
  Widget _buildStatsHeader(bool isDark, int activeCount) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: _statItem('Total', _users.length.toString(), Icons.people_rounded)),
          _statDivider(),
          Expanded(child: _statItem('Active', activeCount.toString(), Icons.check_circle_rounded)),
          _statDivider(),
          Expanded(child: _statItem('Inactive', (_users.length - activeCount).toString(), Icons.block_rounded)),
        ],
      ),
    );
  }
  ...
  Widget _buildBasicInfoTab(bool isDark) {
    ...
    return Form(
      key: _formKey,
      child: ListView(
        padding: EdgeInsets.fromLTRB(16, 20, 16, 16 + mq.padding.bottom),
        children: [
          // Profile avatar
          Center(
            child: Stack(
  ...
  Widget _passwordField(bool isDark) {
    return TextFormField(
      ...
      validator: (v) =>
          (_selectedUser == null && (v == null || v.isEmpty)) ? 'Required' : null,
  ...
  Widget _roleDropdown(bool isDark) {
    return DropdownButtonFormField<String>(
      initialValue: _roles.contains(_selectedRole) ? _selectedRole : _roles.first,
      isExpanded: true,
      items: _roles
    ...
    return ListView(
      ...
      children: [
        ...
        _PermGroup(
          ...
          isDark: isDark,
          items: [
            _PermItem('Create Bills / KOTs', Icons.add_circle_outline_rounded,
                p.canCreateBill, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_create_bill': v}))),
            _PermItem('Edit Bill Items', Icons.edit_rounded,
                p.canEditBill, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_edit_bill': v}))),
            _PermItem('Void Items', Icons.remove_circle_outline_rounded,
                p.canVoidItems, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_void_items': v}))),
            _PermItem('Cancel Bills', Icons.cancel_outlined,
                p.canCancelBill, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_cancel_bill': v}))),
            _PermItem('Apply Discounts', Icons.discount_rounded,
                p.canApplyDiscount, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_apply_discount': v}))),
          ],
        ),
        const SizedBox(height: 14),
        ...
        _PermGroup(
          ...
          isDark: isDark,
          items: [
            _PermItem('View Dashboard', Icons.bar_chart_rounded,
                p.canViewDashboard, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_view_dashboard': v}))),
            _PermItem('View Reports', Icons.assessment_rounded,
                p.canViewReports, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_view_reports': v}))),
          ],
        ),
        const SizedBox(height: 14),
        ...
        _PermGroup(
          ...
          isDark: isDark,
          items: [
            _PermItem('Manage Menu Items', Icons.restaurant_menu_rounded,
                p.canManageItems, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_manage_items': v}))),
            _PermItem('Manage Tables', Icons.table_restaurant_rounded,
                p.canManageTables, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_manage_tables': v}))),
            _PermItem('Manage Users', Icons.people_rounded,
                p.canManageUsers, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_manage_users': v}))),
            _PermItem('System Settings', Icons.settings_rounded,
                p.canManageSettings, (v) => _updatePerm((p) => UserPermission.fromJson({...p.toJson(), 'can_manage_settings': v}))),
          ],
        ),
      ],
    );
